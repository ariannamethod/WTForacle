...
static void load_model(Model* m, const char* path) {
    ...
    // 2. Bigram embedding (uses embed_quant) - optional
    if (c->bigram_vocab > 0) {
        LOAD_EMBED(w->bigram_embed, w->bigram_scales, c->bigram_vocab, n);
    } else {
        w->bigram_embed = NULL; w->bigram_scales = NULL;
    }
